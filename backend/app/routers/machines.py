...
def create_machine(data: MachineCreate, db: Session = Depends(get_db), _user: User = Depends(require_admin)):
...
def update_machine(machine_id: int, data: MachineUpdate, db: Session = Depends(get_db), _user: User = Depends(require_admin)):
...
def delete_machine(machine_id: int, db: Session = Depends(get_db), _user: User = Depends(require_admin)):
    machine = db.get(MachineConfig, machine_id)
    if not machine:
        raise HTTPException(404, "Máquina no encontrada")
    db.delete(machine)
